...
import { BOOK, CAMERA, GROUND, RING } from '../constants';
import { effectiveProgress } from '../store';

// Minimum number of slot spacings visible across the width in the final view.
const FIT_SLOTS = 2.2;
// Share of the viewport width the ring's footprint may span in the overview poses.
const FOOTPRINT_FIT = 0.9;
// Screen space left free for the title (top) and scroll hint (bottom) in the top-down view, and the
// ring's footprint (outer hairline and shadow), which must fit between them.
...
export default function CameraRig({ store }) {
  ...
  const path = useMemo(() => {
    ...
    const ringTop = new Vector3(0, BOOK.height, 0);
    return {
      curve: new CatmullRomCurve3(positions, false, 'centripetal'),
      targets,
      stops: keyframes.map((k) => k.p),
      last,
      // Overview poses frame the whole ring; the final pose frames the front of the ring.
      fitDistances: positions.map((pos, i) => (i === last ? pos.distanceTo(frontSpine) : pos.distanceTo(ringTop))),
  ...
  // An up vector tilted toward -z keeps lookAt stable when the camera is directly above the ring;
  ...
  useFrame((state, delta) => {
    ...
    // Narrow viewports: back each pose off along its view direction until the ring (overview) or
    ...
    for (let i = 0; i <= last; i++) {
      const span = i === last ? FIT_SLOTS * slotSpacing : (2 * GROUND.footprintRadius) / FOOTPRINT_FIT;
      pullBacks[i] = Math.max(0, span / widthPerDistance - fitDistances[i]);
    }

    // Top-down pose: also fit the ring's footprint into the band between the title and the scroll hint.
    const height = state.size.height;
    const band = Math.max(height - INTRO_TOP_PX - INTRO_BOTTOM_PX, height * 0.4);
    const introDistance = (GROUND.footprintRadius * height) / (halfTan * band);
    ...
    // so moving the camera toward −z moves the ring down the screen. Fades out toward the angled view.
    if (k === 0) {
